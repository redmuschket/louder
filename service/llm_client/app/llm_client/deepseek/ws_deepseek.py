...
class DeepSeekWSClient(DeepSeekClient):
    ...
    def __init__(self, chat_id, config_ai: Optional[AiManagerConfig] = None):
        ...
        super().__init__(config_ai=config_ai)
        self._ws_messenger = WsMessenger(chat_id)

    async def ask(self, prompt: str):
        logger.debug(f"___Star DeepSeekWSClient ws ask___")

        logger.debug(f"context.api_uri:{self.context.api_uri}")
        logger.debug(f"context.referer:{self.context.referer}")
        logger.debug(f"context.site_name:{self.context.site_name}")
